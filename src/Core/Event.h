    ...
    */
    ...
    public:
        ...
        EventHandlerImplForMemberFunction(void(U::*memberFunctionToCall)(T &), U *thisPtr)
                : m_pCallerInstance(thisPtr), m_pMemberFunction(memberFunctionToCall) {
        }
      ...
      * @example myEvent += EventHandler::Bind(&ThisClass::OnMessage, this);  // example of binding inside ThisClass constructor as recommended
        ...
        */
        EventBase<T> &operator-=(EventHandlerImpl<T> *pHandlerToRemove) {
            ...
            UpgradeableReadLock handlersReadLock(
                    m_handlersMutex);  // acquire a read lock for the search and switch to a write lock later when the handler is found and is to be deleted.
#endif // SHARP_EVENT_NO_BOOST
    ...
    public:
        ...
        */
        void operator()(T &eventData) {
            ...
            boost::shared_lock<boost::shared_mutex> handlersReadLock(EventBase<T>::m_handlersMutex);
#endif // SHARP_EVENT_NO_BOOST
...
//                if (pHandler) {
//                    pHandler->OnEvent(
//                            eventData);  // this is a virtual function that will eventually call the function passed to Eventhandler::Bind() for this handler
//                }
//            }
            for (auto pHandler: EventBase<T>::m_eventHandlers) {
                if (pHandler) {
                    pHandler->OnEvent(
                            eventData);  // this is a virtual function that will eventually call the function passed to Eventhandler::Bind() for this handler
                }
